...
namespace NDomain.Sql.Tests
{
    ...
    public class SqlEventStoreDbTests
    {
        ...
        [Test]
        public async Task GenericGetAggregates_Should_ReturnAllAvailableAggregatesOfType_When_AggregatesForTypeExist()
        {
            // arrange
            var aggId1 = _fixture.Create<Guid>().ToString();
            var aggId2 = _fixture.Create<Guid>().ToString();
            var aggId3 = _fixture.Create<Guid>().ToString();

            await CreateAggregateWithEvents<TestAggregateRoot>(aggId1).ConfigureAwait(false);
            await CreateAggregateWithEvents<TestAggregateRoot>(aggId2).ConfigureAwait(false);
            await CreateAggregateWithEvents<TestAggregateRoot>(aggId3).ConfigureAwait(false);

            var expectedAggregateIds = new List<string>
            {
                aggId1,
                aggId2,
                aggId3
            ...
            // act
            var aggregateIds = await _store
                .GetAggregates<TestAggregateRoot>()
                .ConfigureAwait(false);

            // assert
            aggregateIds
                .ShouldAllBeEquivalentTo(expectedAggregateIds);
        }

        [Test]
        public async Task GenericGetAggregates_Should_ReturnSpecificAggregatesOfType_When_MultipleTypesOfAggregatesExist()
        {
            // arrange
            var aggId1 = _fixture.Create<Guid>().ToString();
            var aggId2 = _fixture.Create<Guid>().ToString();
            var aggId3 = _fixture.Create<Guid>().ToString();
            var aggId4 = _fixture.Create<Guid>().ToString();
            var aggId5 = _fixture.Create<Guid>().ToString();

            await CreateAggregateWithEvents<TestAggregateRoot>(aggId1).ConfigureAwait(false);
            await CreateAggregateWithEvents<TestAggregateRoot>(aggId2).ConfigureAwait(false);
            await CreateAggregateWithEvents<TestAggregateRoot>(aggId3).ConfigureAwait(false);
            await CreateAggregateWithEvents<TestAggregateRoot2>(aggId4).ConfigureAwait(false);
            await CreateAggregateWithEvents<TestAggregateRoot2>(aggId5).ConfigureAwait(false);
        ...
        [Test]
        public async Task ParameterizedGetAggregates_Should_ReturnAllAvailableAggregatesOfType_When_AggregatesForTypeExist()
        {
            // arrange
            var aggId1 = _fixture.Create<Guid>().ToString();
            var aggId2 = _fixture.Create<Guid>().ToString();
            ...
            // act
            var aggregateIds = await _store
                .GetAggregates(typeof(TestAggregateRoot))
                .ConfigureAwait(false);
        ...
        [Test]
        public async Task ParameterizedGetAggregates_Should_ReturnSpecificAggregatesOfType_When_MultipleTypesOfAggregatesExist()
        {
            // arrange
            var aggId1 = _fixture.Create<Guid>().ToString();
            var aggId2 = _fixture.Create<Guid>().ToString();
